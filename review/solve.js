...
function solveNonogram(horizontalGrid, verticalGrid) {
    ...
    const rowPossibilities = horizontalGrid.map(hints => generatePossibilitiesForRow(hints, numCols));
    const columnPossibilities = verticalGrid.map(hints => generatePossibilitiesForRow(hints, numRows));
    ...
    function isValid(grid, rowPoss, colPoss) {
        for (let r = 0; r < numRows; r++) {
            if (!rowPoss[r].some(pattern => pattern.split('').every((val, idx) => grid[r][idx] === null || grid[r][idx] === parseInt(val))))
                return false;
        }
        for (let c = 0; c < numCols; c++) {
            let column = grid.map(row => row[c]);
            if (!colPoss[c].some(pattern => pattern.split('').every((val, idx) => column[idx] === null || column[idx] === parseInt(val))))
                return false;
        }
        return true;
    }
    ...
    function backtrack(row, col) {
        ...
        for (let num of [0, 1]) {
            grid[row][col] = num;
            if (isValid(grid, rowPossibilities, columnPossibilities) && backtrack(row, col + 1)) {
                return true;
            }
            grid[row][col] = null;
        }
